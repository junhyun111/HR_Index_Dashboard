...
`http://localhost:5080`으로 접속합니다. Development 환경은 현재 Windows 사용자를 모든 권한의 개발 사용자로 자동 로그인시킵니다. 최초 실행 시 `js/innodep_hr_dummy_200.json`의 200건을 `App_Data/hr-dashboard.db`로 한 번만 가져옵니다. 이후 대시보드는 JSON 파일이 아닌 API와 SQLite만 사용합니다.
...
- `Editor`: 데이터 변경
- `Administrator`: 데이터 재가져오기와 외부 연동 상태 확인
...
- `POST /api/admin/import-employees?replace=true`: JSON에서 직원 데이터 재가져오기
...
직원 JSON은 정적 파일 경로로 제공하지 않습니다. 급여 권한이 없는 사용자에게는 API 응답에서도 급여 값과 집계를 반환하지 않으며, 인증된 API 호출은 SQLite의 `AuditEvents` 테이블에 기록됩니다.
