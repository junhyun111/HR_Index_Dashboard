...
const defaultColumnNames={workplace:'사업장',parentDepartment:'상위부서',department:'부서',employeeNumber:'사번',name:'성명',position:'직위',workShift:'근무조',duty:'직책',jobGroup:'직군',employmentType:'사원구분',gender:'성별',education:'최종학력',major:'전공',birthDate:'생년월일',hireDate:'입사일자',terminationDate:'퇴사일자',annualSalary:'연봉'};let columnNames={...defaultColumnNames};
...
function applyColumnNames(rows){document.querySelectorAll('.employee-table th[data-key]').forEach(th=>th.textContent=columnNames[th.dataset.key]||th.textContent);document.querySelectorAll('#employeeForm [name]').forEach(input=>{const label=input.closest('label'),text=[...label.childNodes].find(x=>x.nodeType===Node.TEXT_NODE);if(text)text.nodeValue=`${columnNames[input.name]||defaultColumnNames[input.name]||input.name}${input.name==='annualSalary'?'(원)':''} `;});$('workplaceFilter').options[0].textContent=`전체 ${columnNames.workplace}`;$('deptFilter').options[0].textContent=`전체 ${columnNames.department}`;$('positionFilter').options[0].textContent=`전체 ${columnNames.position}`;$('searchInput').placeholder=`${columnNames.employeeNumber}, ${columnNames.name}, ${columnNames.department}, ${columnNames.duty} 검색`;$('employeeSearchInput').placeholder=`${columnNames.name} 또는 ${columnNames.employeeNumber} 검색`;$('pasteArea').placeholder=rows.sort((a,b)=>a.order-b.order).map(x=>x.displayName).join('\t');document.querySelector('.paste-guide').innerHTML=`Excel 표를 머리글과 함께 붙여넣으세요. <strong>${esc(columnNames.employeeNumber)}만 필수</strong>이며, ${esc(columnNames.employeeNumber)}이 같으면 입력된 값만 업데이트되고 기존 값은 유지됩니다.`;}
...
const val=x=>esc(x||'-'),date=x=>x?x.slice(0,10):'-';function table(a,p){$('employeeRows').innerHTML=a.length?a.map(x=>`<tr><td>${val(x.workplace)}</td><td>${val(x.parentDepartment)}</td><td>${val(x.department)}</td><td>${val(x.employeeNumber)}</td><td>${val(x.name)}</td><td>${val(x.position)}</td><td>${val(x.workShift)}</td><td>${val(x.duty)}</td><td>${val(x.jobGroup)}</td><td>${val(x.employmentType)}</td><td>${val(x.gender)}</td><td>${val(x.education)}</td><td>${val(x.major)}</td><td>${date(x.birthDate)}</td><td>${date(x.hireDate)}</td><td>${date(x.terminationDate)}</td><td>${x.annualSalary==null?'-':fmt.format(x.annualSalary)+'원'}</td></tr>`).join(''):'<tr><td class="table-empty" colspan="17">조건에 맞는 사원이 없습니다.</td></tr>';$('resultCount').textContent=`${fmt.format(p.totalCount)}명`;page=p.page;renderPagination(p.pages);}function esc(v){const d=document.createElement('div');d.textContent=String(v??'');return d.innerHTML;}
...
['workplaceFilter','deptFilter','positionFilter'].forEach(id=>$(id).onchange=()=>{page=1;load();});$('searchInput').oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{page=1;load();},250);};$('resetBtn').onclick=()=>{['workplaceFilter','deptFilter','positionFilter'].forEach(id=>$(id).value='');$('searchInput').value='';page=1;load();};$('pageSizeSelect').onchange=()=>{pageSize=Number($('pageSizeSelect').value);page=1;load();};$('logoutBtn').onclick=async()=>{await fetch('/api/auth/logout',{method:'POST'});location.replace('/login');};
const employeeFields=['employeeNumber','name','workplace','parentDepartment','department','position','workShift','duty','jobGroup','employmentType','gender','education','major','annualSalary','birthDate','hireDate','terminationDate'];let employeeSearchMode='edit',employeeSearchTimer;
...
$('excelUploadInput').onchange=async()=>{
  ...
  if(!confirm(`${file.name} 파일을 ${displayDate} 직원 현황에 반영할까요?`))return;
  ...
  try{
    ...
    try{response=await fetch('/api/employees/import',{method:'POST',body,signal:controller.signal});}finally{clearTimeout(timeout);}
    const result=await response.json().catch(()=>({}));
    if(!response.ok)throw Error(result.message||`업로드 실패(HTTP ${response.status})`);
    $('importStatus').textContent=`${selected} Excel 반영 완료 · 수정 ${result.updated}명 / 추가 ${result.added}명`;
    $('pasteDialog').close();initialized=false;['workplaceFilter','deptFilter','positionFilter'].forEach(id=>$(id).options.length=1);page=1;await load();
  }catch(error){
