...
function applyColumnNames(rows){document.querySelectorAll('.employee-table th[data-key]').forEach(th=>th.textContent=columnNames[th.dataset.key]||th.textContent);document.querySelectorAll('#employeeForm [name]').forEach(input=>{const label=input.closest('label'),text=[...label.childNodes].find(x=>x.nodeType===Node.TEXT_NODE);if(text)text.nodeValue=`${columnNames[input.name]||defaultColumnNames[input.name]||input.name}${input.name==='annualSalary'||input.name==='monthlyWage'?'(원)':''} `;});$('workplaceFilter').options[0].textContent=`전체 ${columnNames.workplace}`;$('deptFilter').options[0].textContent=`전체 ${columnNames.department}`;$('positionFilter').options[0].textContent=`전체 ${columnNames.position}`;$('searchInput').placeholder=`${columnNames.employeeNumber}, ${columnNames.name}, ${columnNames.department}, ${columnNames.duty} 검색`;$('employeeSearchInput').placeholder=`${columnNames.name} 또는 ${columnNames.employeeNumber} 검색`;$('pasteArea').placeholder=rows.sort((a,b)=>a.order-b.order).map(x=>x.displayName).join('\t');document.querySelector('.paste-guide').innerHTML=`Excel 표를 머리글과 함께 붙여넣으세요. <strong>${esc(columnNames.employeeNumber)}만 필수</strong>이며, 빈 값이나 형식이 맞지 않는 값은 NULL로 저장됩니다.`;}
