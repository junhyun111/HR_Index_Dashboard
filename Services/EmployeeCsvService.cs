...
{
    public static readonly string[] Headers=["사업장","상위부서","부서","사번","성명","직위","근무조","직책","직군","사원구분","성별","최종학력","전공","생년월일","입사일자","퇴사일자","연봉"];
    ...
    public byte[] Export(IReadOnlyCollection<Employee> employees)
    {
        ...
        foreach(var x in employees) { string[] values=[x.Workplace??"",x.ParentDepartment??"",x.Department??"",x.EmployeeNumber,x.Name??"",x.Position??"",x.WorkShift??"",x.Duty??"",x.JobGroup??"",x.EmploymentType??"",x.Gender??"",x.Education??"",x.Major??"",Date(x.BirthDate),Date(x.HireDate),Date(x.TerminationDate),x.AnnualSalary?.ToString(CultureInfo.InvariantCulture)??""]; csv.AppendLine(string.Join(',',values.Select(Escape))); }
    ...
    private static EmployeeImportResult Parse(TextFieldParser parser,string source,IReadOnlyDictionary<string,string>? headerAliases)
    {
        ...
        while(!parser.EndOfData){n++;try{var f=parser.ReadFields()??[];if(f.All(string.IsNullOrWhiteSpace))continue;string V(string h)=>map.TryGetValue(h,out var i)&&i<f.Length?f[i].Trim():"";string? O(string h,int max){var v=V(h);if(v.Length>max)throw new EmployeeCsvException($"{n}행 [{h}]: {max}자를 초과했습니다.");return v.Length==0?null:v;}var no=V("사번");if(no.Length==0)throw new EmployeeCsvException($"{n}행 [사번]: 값이 비어 있습니다.");if(no.Length>50)throw new EmployeeCsvException($"{n}행 [사번]: 50자를 초과했습니다.");rows.Add(new(no,O("사업장",100),O("상위부서",100),O("부서",100),O("성명",100),O("직위",50),O("근무조",50),O("직책",50),O("직군",50),O("사원구분",50),O("성별",20),O("최종학력",100),O("전공",100),ParseDate(V("생년월일"),n,"생년월일"),ParseDate(V("입사일자"),n,"입사일자"),ParseDate(V("퇴사일자"),n,"퇴사일자"),ParseSalary(V("연봉"),n)));}catch(Exception e)when(e is EmployeeCsvException or MalformedLineException){errors.Add(e.Message);}}
    ...
    private static void WriteSheet(XmlWriter writer,IReadOnlyCollection<Employee> employees,IReadOnlyDictionary<string,string>? displayNames)
    {
        ...
        writer.WriteStartElement("cols");double[] widths=[14,18,18,13,12,12,12,12,12,14,10,14,18,13,13,13,16];for(var i=0;i<widths.Length;i++){writer.WriteStartElement("col");writer.WriteAttributeString("min",(i+1).ToString());writer.WriteAttributeString("max",(i+1).ToString());writer.WriteAttributeString("width",widths[i].ToString(CultureInfo.InvariantCulture));writer.WriteAttributeString("customWidth","1");writer.WriteEndElement();}writer.WriteEndElement();
        ...
        var row=2;foreach(var x in employees){writer.WriteStartElement("row");writer.WriteAttributeString("r",row.ToString());string?[] text=[x.Workplace,x.ParentDepartment,x.Department,x.EmployeeNumber,x.Name,x.Position,x.WorkShift,x.Duty,x.JobGroup,x.EmploymentType,x.Gender,x.Education,x.Major];for(var i=0;i<text.Length;i++)TextCell(writer,Cell(i,row),text[i]??"");DateCell(writer,Cell(13,row),x.BirthDate);DateCell(writer,Cell(14,row),x.HireDate);DateCell(writer,Cell(15,row),x.TerminationDate);NumberCell(writer,Cell(16,row),x.AnnualSalary);writer.WriteEndElement();row++;}
        writer.WriteEndElement();writer.WriteStartElement("autoFilter");writer.WriteAttributeString("ref",$"A1:Q{Math.Max(1,row-1)}");writer.WriteEndElement();writer.WriteEndElement();
    }
...
public sealed record EmployeeImportRow(string EmployeeNumber,string? Workplace,string? ParentDepartment,string? Department,string? Name,string? Position,string? WorkShift,string? Duty,string? JobGroup,string? EmploymentType,string? Gender,string? Education,string? Major,DateTime? BirthDate,DateTime? HireDate,DateTime? TerminationDate,long? AnnualSalary);
