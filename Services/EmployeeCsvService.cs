...
{
    public static readonly string[] Headers=["사업장","상위부서","부서","사번","성명","직위","근무조","직책","직군","사원구분","성별","생년월일","입사일자","퇴사일자"];
    private static readonly string[] RequiredHeaders=Headers.Where(x=>x!="생년월일").ToArray();
    public byte[] Export(IReadOnlyCollection<Employee> employees)
    {
        ...
        foreach(var x in employees) { string[] values=[x.Workplace??"",x.ParentDepartment??"",x.Department??"",x.EmployeeNumber,x.Name??"",x.Position??"",x.WorkShift??"",x.Duty??"",x.JobGroup??"",x.EmploymentType??"",x.Gender??"",Date(x.BirthDate),Date(x.HireDate),Date(x.TerminationDate)]; csv.AppendLine(string.Join(',',values.Select(Escape))); }
...
public sealed record EmployeeImportRow(string EmployeeNumber,string? Workplace,string? ParentDepartment,string? Department,string? Name,string? Position,string? WorkShift,string? Duty,string? JobGroup,string? EmploymentType,string? Gender,DateTime? BirthDate,DateTime? HireDate,DateTime? TerminationDate);
