...
public sealed class EmployeeCsvService
{
    ...
    public byte[] ExportExcel(IReadOnlyCollection<Employee> employees)
    {
        ...
        using(var archive=new ZipArchive(output,ZipArchiveMode.Create,true))
        {
            ...
            WriteXml(archive,"xl/worksheets/sheet1.xml",writer=>WriteSheet(writer,employees));
        }
    ...
    private static void WriteSheet(XmlWriter writer,IReadOnlyCollection<Employee> employees)
    {
        const string ns="http://schemas.openxmlformats.org/spreadsheetml/2006/main";
        ...
        writer.WriteStartElement("sheetData");writer.WriteStartElement("row");writer.WriteAttributeString("r","1");for(var i=0;i<Headers.Length;i++)TextCell(writer,Cell(i,1),Headers[i],1);writer.WriteEndElement();
