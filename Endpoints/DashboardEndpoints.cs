...
public static class DashboardEndpoints
{
        ...
        string? sort, string? direction, int page, int pageSize, AppDbContext db, CancellationToken ct)
    {
        ...
        return Results.Ok(new {
            filters=new { workplaces=await Values(db.Employees.Select(x=>x.Workplace),ct), departments=await Values(db.Employees.Select(x=>x.Department).Concat(db.Employees.Select(x=>x.ParentDepartment)),ct), genders=await Values(db.Employees.Select(x=>x.Gender),ct) },
